...
/// Day bucketing and labelling for scheduled games.
///
...
nonisolated enum SportsClock {
    ...
    /// Guards against the parse-failure sentinels the two mappers use
    /// (`Date()` in `mapRow`, `.distantPast` in the ESPN path).
    static func isUsable(_ date: Date) -> Bool {
        date.timeIntervalSince1970 > 0 && date.timeIntervalSinceNow < 60 * 60 * 24 * 400
    }

    static func isToday(_ date: Date) -> Bool {
        isUsable(date) && calendar.isDateInToday(date)
    }
    ...
    static func label(for date: Date) -> String? {
        guard isUsable(date) else { return nil }
...
extension SportsGame {
    ...
    var scheduleLabel: String {
        ...
        return SportsClock.label(for: startDate) ?? statusDetail
    }

    /// True when the game starts today in Eastern time.
    var startsToday: Bool { SportsClock.isToday(startDate) }
}
