...
struct NotificationsSettingsView: View {
    ...
    /// when the user wants it (`auth.notifyPushEnabled`) *and* iOS has granted
    ...
    /// iOS authorization is `.notDetermined` while the account still wants
    /// push — the delete/reinstall case. The grant is revoked with the app, so
    /// the prompt has to be shown again.
    ...
    var body: some View {
                ...
                VStack(spacing: 16) {
                    if systemDenied {
                        ...
                        statusBanner(
                            ...
                            message: "iOS clears notification permission when an app is deleted. Flip the switch below to allow alerts again — your alert types are still saved."
                        )
                    }
        ...
        // toggle is reconciled against the iOS grant, otherwise the two
        ...
        .onChange(of: scenePhase) { _, phase in
            // Catches a grant (or revocation) made in iOS Settings while the
    ...
    private func handlePushToggle(_ newValue: Bool) {
        ...
        Task { @MainActor in
            ...
            // iOS never re-prompts once denied — asking again silently returns
            // false, so route the user to Settings instead of flipping their
            // saved intent off.
    ...
    /// Reconciles the master toggle with iOS. The OS grant is the source of
    /// truth for whether a push can actually arrive — deleting the app revokes
    /// it — so a server-restored `notify_push = true` shows as off until the
    /// user re-grants, and the re-auth banner explains why. The saved intent is
    /// never overwritten here, so the account's alert types survive intact.
