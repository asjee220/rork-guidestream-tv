...
struct TVHeroCarousel: View {
    ...
    /// False until focus has actually left the hero once. Until then the
    /// metadata block stays visible: tvOS will not move focus onto a fully
    /// transparent view, so gating visibility purely on hero focus made the
    /// CTA unfocusable and Home opened on the first rail card instead.
